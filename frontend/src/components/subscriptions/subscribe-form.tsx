...
import { Check, Loader2, Refrigerator, Snowflake, Tag, X, Zap } from "lucide-react";
    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
      ...
      <aside className="h-fit rounded-2xl border border-slate-200 bg-sky-50 p-6">
        ...
        <p className="mt-3 text-sm font-semibold text-brand-700">
          {coupon ? (
            <>
              <span className="mr-1.5 text-slate-400 line-through">&pound;{coupon.originalAmount}</span>
              &pound;{coupon.discountedAmount}
            </>
          ) : (
            <>&pound;{plan.price.amount}</>
          )}{" "}
          for {plan.months} month{plan.months > 1 ? "s" : ""} &middot;{" "}
          {plan.servicesPerCycle} service{(plan.servicesPerCycle ?? 0) > 1 ? "s" : ""} included
        </p>
        <p className="mt-1 text-xs text-slate-400">
          Works out at &pound;{plan.pricePerMonth.toFixed(2)}/month
        </p>
        ...
        {plan.addOnMonthlyPrice && (
          <p className="mt-4 rounded-lg bg-white px-3 py-2.5 text-xs leading-relaxed text-slate-500">
            After your {plan.months}-month term you can extend month-by-month for
            &pound;{plan.addOnMonthlyPrice}/month.
          </p>
        )}
